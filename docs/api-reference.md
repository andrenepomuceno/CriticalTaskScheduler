...
Auto-detected on **ESP32** (`ARDUINO_ARCH_ESP32`), **RP2040** (`ARDUINO_ARCH_RP2040`, arduino-pico core), and **nRF52** (`ARDUINO_ARCH_NRF52`, Adafruit core) — FreeRTOS is bundled in their cores.

**STM32** and **Teensy 4.x** require manual opt-in: add the appropriate FreeRTOS library to `lib_deps` (e.g. `stm32duino/STM32FreeRTOS` for STM32) and set `-D CRITICALTASKSCHEDULER_HAS_FREERTOS=1` in your build flags.
